...
priority: u8,
state: TaskState,
context: system.threading.TaskContext,
...
pub fn format(self: *const @This(), comptime _: []const u8, _: std.fmt.FormatOptions, fmt: anytype) !void {
    try fmt.print("process {s} ({}) task {} - priority {} - {s}", .{ self.process.name, self.process.process_id, self.task_id, self.priority, @tagName(self.state) });
